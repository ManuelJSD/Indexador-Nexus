...
public class AdapterController {
    ...
    private void btnAdapter_OnAction() {
        ...
        try {
            ...
            showAlert(AlertType.ERROR, "Formato Inválido", "La 'Primera posición' debe ser un número entero válido.");
            return;
        }

        try {
            if (txtGrafico.getText().trim().isEmpty()) {
                showAlert(AlertType.WARNING, "Falta Gráfico", "Debes especificar el 'Nº Gráfico' de destino.");
                return;
            }
            graphIndex = Long.parseLong(txtGrafico.getText().trim());
        } catch (NumberFormatException e) {
            showAlert(AlertType.ERROR, "Formato Inválido", "El 'Nº Gráfico' debe ser un número entero válido.");
            return;
        }
        ...
        for (String linea : lineas) {
            ...
            try {
                ...
                if (frames == 1) {
                    ...
                    resultado.append("Grh").append(contador).append("=")
                            .append("1-").append(graphIndex).append("-") // Reemplazamos FileNum
                    ...
                    try {
                        ...
                        showAlert(AlertType.ERROR, "Formato Inválido", "El 'Nº Frames' debe ser un número entero.");
                        return;
                    }
