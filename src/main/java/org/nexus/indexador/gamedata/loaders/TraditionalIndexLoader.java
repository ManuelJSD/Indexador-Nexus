...
public class TraditionalIndexLoader implements IndexLoader {
    ...
    private void saveIndFile(String filename, String formatKey, SaveAction action) throws IOException {
        File archive = new File(configManager.getInitDir() + filename);
        IndFileFormat format = detectedFormats.get(formatKey);
        long offset = (format != null) ? format.getDataOffset() : (filename.equals("graficos.ind") ? 263 : 0);
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(archive))) {
            ...
            for (GrhData grh : entries) {
                ...
                if (grh.getNumFrames() > 1) {
                    ...
                    sb.append("-").append(grh.getFileNum())
                            .append("-").append(grh.getsX())
                            .append("-").append(grh.getsY())
                            .append("-").append(grh.getTileWidth())
                            .append("-").append(grh.getTileHeight());
                }
